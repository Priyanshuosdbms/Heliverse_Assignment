...
Every register in the input must appear in the output. Do not merge, collapse, skip, or reorder registers. Map each input register to exactly one `register` definition + instantiation.

### R2: Preserve All Field Information

Every field (bit range) in the input must appear as a `field` in the corresponding `register`. Use the exact bit positions from the input. If the input specifies a name, desc, sw access, hw access, reset value, or side-effect behavior, encode them all explicitly.
...
Every `register` must declare `regwidth` explicitly:
```systemrdl
register {
    name = "control";
    regwidth = 32;
...
### R5: Always Emit `sw` and `hw` on Every Field — Use Quoted Strings

Never rely on default access type inference. Every `field` must have explicit `sw` and `hw` declarations. **Per the PeakRDL-Renode README, access type values must be quoted strings, not bare identifiers:**
...
external mem {
    ...
    register {
        name = "entry";
        desc = "Single FIFO entry, 32-bit data.";
        regwidth = 32;

        field {
            name = "data";
            desc = "32-bit data payload.";
            sw = "rw";
            hw = "rw";
        } data [31:0] = 0;
...
- `regfile` names: PascalCase (e.g., `UartCore`)
- `register` instance names: snake_case (e.g., `ctrl`, `status`, `txdr`)
- `field` instance names: snake_case (e.g., `enable`, `busy`, `fifo_level`)
- `enum` type names: snake_case with `_e` suffix (e.g., `power_state_e`)
- `name` property values inside bodies: use the same string as the instance name, lowercase (e.g., `name = "status";` for instance `status`)
- The PeakRDL-Renode plugin converts register instance names to CamelCase (`status` → `this.Status`) and field instance names to UPPER_CASE (`busy` → `BUSY`) automatically.

### R19: No Comments Inside Register or Field Bodies

Do not place inline comments inside `register { }` or `field { }` bodies. All explanatory text belongs in the `name` and `desc` properties, which is exactly what those properties exist for.

Comments are permitted only at the structural level — before a `register` instantiation line, before a `regfile` block, or at the top of the file — and only to note assumptions made about the input when data was ambiguous or missing. Do not use comments as a substitute for a missing `desc`.

### R20: Validate Structural Completeness

Before emitting the final output, verify:
1. Every register from the input has a corresponding `register` with an explicit address.
2. Every field has explicit `sw` (quoted string), `hw` (quoted string), bit range with space before `[`, and reset value.
...
**Properties**
- [ ] Every `register` has `regwidth` explicitly set
- [ ] Every `register` has `name` and `desc`
- [ ] Every `field` has explicit `sw` and `hw` as **quoted strings** (e.g. `sw = "rw"`)
...
The following examples use the **exact syntax accepted by PeakRDL-Renode** as shown in the official README. Key conventions that differ from the raw SystemRDL 2.0 spec:

- Use `register` keyword (not `reg`) for register definitions inside an `addrmap` or `regfile`.
- `sw` and `hw` values are **quoted strings**: `sw = "rw"`, `hw = "r"`, not bare identifiers.
- Field instantiation has a space before the bit range: `busy [0:0]` not `busy[0:0]`.
- `name` values inside `register` and `field` bodies are lowercase quoted strings matching the instance name.
- No inline comments inside `register` or `field` bodies.
- Reset values on 1-bit fields: plain `= 0` or `= 1`. Multi-bit non-zero: `= N'hXX`.
...
### Example 2: Read/write control register with callbacks
...
Always generate all three registers together for any interrupt subsection in the input.
...
Use when the input describes a memory-mapped buffer, FIFO, or RAM region. The `mem` must always be declared `external`.
...
### Example 5: Register array (large repeated register block)

Use this pattern when the input describes N identical registers at a strided offset (e.g. `offset = base + 8*n`). A plain register array at `addrmap` level is **not supported** by PeakRDL-Renode — the array must be wrapped inside an `external mem` node.

**Critical sizing rule:** `memwidth` must match your **stride in bits**, not the register's field width. If registers are 32-bit but stride is 8 bytes (64 bits), set `memwidth = 64`. The tool validates that `mementries × (memwidth/8)` exactly covers your address range.

**Known tool bug at base address 0x0:** When the `mem` is placed at offset `0x0`, the generated C# dispatch contains a constant-folding bug:
```csharp
if(offset >= 0 && offset < 0L)   // bug: should be < 0L + RegisterBank.Size
```
This condition never matches, silently dropping all accesses. The workaround is to either place the `mem` at a non-zero offset (e.g. `@ 0x8`) **or** patch the generated file after running `peakrdl renode`:
```bash
sed -i 's/offset < 0L)/offset < 0L + RegisterBank.Size)/' YourPeripheral_gen.cs
```

**RDL for 231 registers, each 32-bit, at stride 8 bytes (offsets 0x0 to 0x730):**

```systemrdl
addrmap MyPeripheral {
    name = "MyPeripheral";
    desc = "Peripheral with a 231-entry register array at 8-byte stride.";

    external mem {
        name = "register_bank";
        desc = "Array of 231 identical registers. Each entry is 32-bit wide at an 8-byte stride.";
        mementries = 231;
        memwidth = 64;

        register {
            name = "register_name";
            desc = "Single array entry. Contains one 32-bit data field.";
            regwidth = 32;

            field {
                name = "field1";
                desc = "32-bit data field for this array entry.";
                sw = "rw";
                hw = "rw";
            } field1 [31:0] = 0;
...
public partial class MyPeripheral
{
    ...
    {
        uint val = RegisterBank[5].FIELD1;
        RegisterBank[5].FIELD1 = 0x1234;
    }
}
```
...
```bash
peakrdl renode -n MyPeripheral -N MyNamespace -o MyPeripheral_gen.cs peripheral.rdl
```

After generation, if the `mem` base is `0x0`, apply the dispatch patch:
```bash
sed -i 's/offset < 0L)/offset < 0L + RegisterBank.Size)/' MyPeripheral_gen.cs
```

---

### Example 6: Full peripheral — regfile grouping, enum, mixed register types
...
addrmap SimpleUART {
    ...
    regfile UartCore {
        ...
        register {
            ...
            field {
                name = "txdata";
                desc = "Eight-bit data byte to transmit. Writing this field enqueues the byte and self-clears after one cycle.";
                sw = "w";
                hw = "r";
                singlepulse;
            } txdata [7:0] = 0;
        ...
        register {
            name = "bdr";
            desc = "Baud rate divisor register. BaudRate = ClkFreq / (16 * (divisor + 1)). Configure while enable is 0.";
            regwidth = 32;

            field {
                name = "divisor";
                desc = "Sixteen-bit baud rate divisor.";
                sw = "rw";
                hw = "r";
        ...
        register {
            name = "lcr";
            desc = "Line control register. Configures the UART frame format. Configure while enable is 0.";
            regwidth = 32;
            ...
            field {
                name = "stopbits";
                desc = "Stop bit count. 0 = one stop bit, 1 = two stop bits.";
                sw = "rw";
                hw = "r";
            } stopbits [2:2] = 0;
        ...
        register {
            ...
            field {
                name = "enable";
                desc = "Master enable. Write 1 to start the UART engine. Must be 0 when reconfiguring bdr or lcr.";
                sw = "rw";
                hw = "r";
            } enable [0:0] = 0;
    ...
    regfile UartIntr {
        ...
        register {
            name = "isr";
            desc = "Interrupt status register. Set by hardware, cleared via icr.";
            regwidth = 32;
            default sw = "r";
            default hw = "w";
        ...
        register {
            name = "icr";
            desc = "Interrupt clear register. Write 1 to a bit to clear the corresponding status flag in isr. Always reads as 0.";
            regwidth = 32;
