...
class RegArray:
    ...
    @staticmethod
    def m_cs_type_name(ty: ast.Type) -> str:
        if ty == ast.Type.bool: return 'bool'
        if ty == ast.Type.byte: return 'byte'
        if ty == ast.Type.ushort: return 'ushort'
        if ty == ast.Type.uint: return 'uint'
        if ty == ast.Type.ulong: return 'ulong'
        raise RuntimeError(f'No C# type name mapping for {ty}')

    @staticmethod
    def m_value_provider_callback_name(field: FieldNode) -> str:
        return field.inst_name.upper() + '_ValueProviderCallback'

    @staticmethod
    def m_write_callback_name(field: FieldNode) -> str:
        return field.inst_name.upper() + '_WriteCallback'
    ...
    def m_generate_underlying_property(field: FieldNode) -> ast.VariableDecl:
        ...
        cs_ty = RegArray.m_cs_type_name(field_type)
        vpc_name = RegArray.m_value_provider_callback_name(field)
        wc_name = RegArray.m_write_callback_name(field)
        ...
        def generate_getter_body() -> ast.Stmt:
            # If the user has wired a ValueProviderCallback (from the
            # peripheral's Init(), same pattern as regular reg fields),
            # it fully overrides the default backing-store read for this
            # index -- matching Renode's own field callback convention.
            callback_check = ast.If(
                condition = ast.HardExpr(f'parent.{vpc_name} != null', ty=ast.Type.bool),
                then = ast.Return(ast.HardExpr(f'parent.{vpc_name}(index)', ty=field_type))
            )
            return ast.Node.join([
                callback_check,
                get_tempvar,
        ...
        def generate_setter_body() -> ast.Stmt:
            # Capture the pre-write value (going through the getter, so it
            # reflects any ValueProviderCallback override), perform the
            # normal masked byte-packed write as before, then notify a
            # WriteCallback if one has been wired, passing (index, old, new)
            # -- same (oldval, newval) shape as a regular reg field's
            # WriteCallback, plus the array index.
            old_value_decl = ast.VariableDecl(
                'oldValue', field_type,
                init = ast.HardExpr(f'this.{field.inst_name.upper()}', ty=field_type)
            )
            notify = ast.HardExpr(
                f'parent.{wc_name}?.Invoke(index, oldValue, value)',
                ty = ast.Type('void')
            ).into_stmt()
            return ast.Node.join([
                old_value_decl,
                generate_setter_assignments(),
                notify
            ])
        ...
        return ast.PropertyDefintion(
            ...
            get = generate_getter_body(),
            set = generate_setter_body(),
            ret_ty = field_type
        )
    ...
    def generate_csharp_wrapper_type(self) -> ast.Class:
        ...
        return ast.Class(
            ...
            fields = ast.Node.join([
                ...
                ast.VariableDecl('parent', ast.Type(self.type_name), access=PRIVATE),
                ast.VariableDecl('index', ast.Type.long, access=PRIVATE)
            ]),
            ...
            methods = ast.MethodDefinition(
                ...
                args = ast.ArgDecl('memory', ast.Type.byte.array())
                    ...
                    .then(ast.ArgDecl('parent', ast.Type(self.type_name)))
                    .then(ast.ArgDecl('index', ast.Type.long)),
                body = ast.Node.join([
                    ast.Assign(
                    ...
                    ast.Assign(
                        lhs = ast.HardExpr('this.parent', ty=ast.Type(self.type_name)),
                        rhs = ast.HardExpr('parent', ty=ast.Type(self.type_name))
                    ).into_stmt(),
                    ast.Assign(
                        lhs = ast.HardExpr('this.index', ty=ast.Type.long),
                        rhs = ast.HardExpr('index', ty=ast.Type.long)
                    ).into_stmt(),
                ])
            )
        )
        ...
        return ast.Class(
            ...
            fields = ast.Node.join([
                ...
                *chain.from_iterable(
                    (
                        ast.VariableDecl(
                            RegArray.m_value_provider_callback_name(field),
                            ast.Type(f'System.Func<long, {RegArray.m_cs_type_name(RegArray.m_get_underlying_field_type(field))}>'),
                            access = PUBLIC,
                            doc = f'Overrides the read value of "{field.inst_name.upper()}" for any index, if set.'
                        ),
                        ast.VariableDecl(
                            RegArray.m_write_callback_name(field),
                            ast.Type(
                                'System.Action<long, '
                                f'{RegArray.m_cs_type_name(RegArray.m_get_underlying_field_type(field))}, '
                                f'{RegArray.m_cs_type_name(RegArray.m_get_underlying_field_type(field))}>'
                            ),
                            access = PUBLIC,
                            doc = f'Called as (index, oldValue, newValue) after software writes "{field.inst_name.upper()}", if set.'
                        )
                    )
                    for field in self.register.fields()
                )
