...
@tool
def review_column_mapping(inspection_json: str, grammar_spec_json: str) -> str:
    """HUMAN CHECKPOINT 1 — show the user a table of detected column mappings
    and let them approve or correct before codegen runs.

    Prints a formatted table of every sampled column with its header value
    and what the schema sniffer assigned it to (address, msb, lsb, etc.).
    The user can type 'y' to approve, or provide corrections as a JSON dict
    of {field_name: new_col_index} overrides.

    Returns a JSON string of the (possibly corrected) grammar spec. Always
    pass this return value — not the original grammar_spec_json — into the
    codegen sub-agent.

    inspection_json:  raw output from inspect_excel.
    grammar_spec_json: JSON string from the schema-sniffer sub-agent.
    """
    try:
        inspection   = json.loads(inspection_json)
        grammar_spec = json.loads(grammar_spec_json)
    except json.JSONDecodeError as e:
        return json.dumps({"error": f"JSON parse error: {e}", "grammar_spec": grammar_spec_json})
...
agent = create_deep_agent(
    ...
    system_prompt=(
        ...
        "  Call review_column_mapping with the raw inspection JSON and the grammar "
        "  spec JSON string. This shows the user a table of detected column mappings "
        "  and pauses for their approval or corrections. Use the JSON it returns "
        "  (not the original grammar spec) for all subsequent steps.\n\n"
