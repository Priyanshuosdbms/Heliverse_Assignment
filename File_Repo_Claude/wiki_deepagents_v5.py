...
fix-coverage addresses the pipeline's real coverage gap: the alias/citation
pre-scan can only catch a relationship if at least one side already has a
declared alias. It finds every concept with zero declared aliases (audit_
alias_coverage), then has the agent propose real aliases for up to `limit`
of them (default 15) per run — checking each proposal against the ambiguity
registry via lookup_alias first, so closing a coverage gap doesn't silently
introduce a new ambiguous alias.

lint also now tracks a llm-inferred confidence-share metric over time
(wiki/_metrics.jsonl, one line per lint run) — a rising share across ingests
is a smell worth investigating, not proof of a specific missed relationship,
but the closest thing this pipeline has to a coverage trend signal.
