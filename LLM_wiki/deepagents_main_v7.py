...
What's fully implemented from the planning doc:
  ...
  NEW       Mid-pipeline coverage review — check_chunk_coverage() runs right
            after EACH chunk (not as a post-hoc pass over the finished
            wiki), using compare_json_to_wiki's page-provenance logic
            scoped to just that chunk. Any gap is delegated to a dedicated
            coverage-reviewer-agent subagent that REASONS about whether the
            omission was intentional (boilerplate, running headers, a pure
            restatement filed elsewhere) or a genuine miss — and self-heals
            genuine misses on the spot rather than only reporting them.
            Every review is permanently logged via log_coverage_review to
            wiki/_coverage-reviews.jsonl, verdict + actual reasoning +
            action taken. chunk_sections() also now explicitly flags (never
            silently swallows) a section whose own content alone exceeds
            the chunk budget — that context is passed to the reviewer as a
            plausible, legitimate explanation to weigh before concluding a
            gap was a genuine miss.
...
import compare_json_to_wiki as coverage  # same-directory module — page/content coverage checks, now reused mid-pipeline instead of only post-hoc
...
COVERAGE_REVIEW_LOG_PATH = WIKI_ROOT / "_coverage-reviews.jsonl"
...
# ---- mid-pipeline coverage check (moved up from post-hoc) ------------------
# Runs after EACH chunk is ingested, scoped to just that chunk's own pages —
# not the whole document — so a gap is caught (and possibly self-healed)
# while the relevant source content is still in a fresh context, rather than
# only surfacing in a report after the entire spec has already been ingested.

def check_chunk_coverage(chunk: list[dict], oversized_section_ids: list[str]) -> list[dict]:
    """Deterministic (no LLM call): using compare_json_to_wiki's own
    page-provenance logic, find every page THIS chunk references that no
    wiki page (written so far, including by this very chunk) claims via
    source_pages. Returns a list of {page, contexts, blocks, in_oversized_section}."""
    expected_pages = coverage.extract_json_pages(chunk)
    wiki_pages = coverage.build_wiki_page_index(WIKI_ROOT)

    gaps = []
    for page, contexts in expected_pages.items():
        if page in wiki_pages:
            continue
        section_ids = {c.get("section") for c in contexts if c}
        gaps.append({
            "page": page,
            "contexts": sorted({(c.get("section"), c.get("title")) for c in contexts if c}),
            "blocks": coverage.get_blocks_for_page(chunk, page),
            "in_oversized_section": bool(section_ids & set(oversized_section_ids)),
        })
    return sorted(gaps, key=lambda g: g["page"])


def log_coverage_review(page: int, section: str, verdict: str, reasoning: str, action_taken: str = "none") -> str:
    """Append a structured, permanent record of a coverage-gap review to
    wiki/_coverage-reviews.jsonl. Call this once per page you review, AFTER
    you've decided whether the gap was intentional or a genuine miss — this
    is the audit trail the person asked for; do not skip logging a review
    just because you fixed the gap yourself.

    Args:
        page: the page number that was missing.
        section: the spec section id it belongs to.
        verdict: "intentional" or "genuine_miss" — nothing else.
        reasoning: your actual reasoning, in your own words — not a template.
        action_taken: "none", or a description of what you filed/fixed, e.g.
            "filed as /commands/admin/foo.md".
    """
    from datetime import datetime
    if verdict not in ("intentional", "genuine_miss"):
        return f"REJECTED: verdict must be 'intentional' or 'genuine_miss', got {verdict!r}"
    record = {
        "timestamp": datetime.now().isoformat(timespec="seconds"),
        "page": page, "section": section, "verdict": verdict,
        "reasoning": reasoning, "action_taken": action_taken,
    }
    with COVERAGE_REVIEW_LOG_PATH.open("a", encoding="utf-8") as f:
        f.write(json.dumps(record) + "\n")
    return f"logged: page {page} -> {verdict}"


def review_chunk_coverage(gaps: list[dict], chunk_index: int) -> None:
    """Delegates each chunk's coverage gaps to the coverage-reviewer-agent
    subagent for actual reasoning about intentional-vs-missed, rather than
    just recording the gap as a bare fact. The reviewer can self-heal a
    genuine miss (it has the same write tools as entity-ingest-agent) or
    conclude the omission was fine (boilerplate, running headers, pure
    cross-references already covered elsewhere) — either way it MUST call
    log_coverage_review so the reasoning is captured permanently, not just
    left in this turn's transcript."""
    if not gaps:
        return

    print(f"[coverage] chunk {chunk_index}: {len(gaps)} page(s) with no wiki coverage yet — requesting review")

    gap_descriptions = []
    for g in gaps:
        note = " (NOTE: this page's section exceeded the chunk token budget on its own — that overflow is a plausible, legitimate explanation worth weighing before concluding it was a genuine miss)" if g["in_oversized_section"] else ""
        gap_descriptions.append(
            f"- page {g['page']}{note}, in section(s) {g['contexts']}\n"
            f"  raw content found on this page: {json.dumps(g['blocks'])[:1500]}"
        )

    instruction = (
        f"Delegate this to the coverage-reviewer-agent subagent via the task tool. "
        f"The following page(s) from the batch just ingested have NO wiki concept "
        f"claiming them in source_pages, meaning they may have been entirely dropped "
        f"during ingestion:\n\n" + "\n\n".join(gap_descriptions) +
        "\n\nFor each page, the reviewer must reason (in its own words, not a template) "
        "about whether this is genuinely fine (e.g. the page's content is a running "
        "header/footer, a table of contents entry, a page break with no new "
        "information, or content that's a pure restatement of something already "
        "filed under a different page) versus a real miss (the page contains "
        "substantive spec content — a command, field definition, table row, or "
        "behavior description — that should have its own or an updated wiki entry "
        "but doesn't). If it's a genuine miss, the reviewer should attempt to file "
        "it properly right now (check_duplicates first, then write_wiki_page), not "
        "just report it. Either way, call log_coverage_review exactly once per page "
        "with your real reasoning and what action you took."
    )
    print(run(instruction))
...
SHARED_TOOLS = [
    ...
    extract_document, lint_wiki, log_coverage_review,
]
...
COVERAGE_REVIEWER_PROMPT = (
    "You review pages the wiki appears to have skipped during ingestion. You "
    "are given the raw source content for one or more pages and told no wiki "
    "concept currently claims them. Your job is to REASON about why, in your "
    "own words — not to apply a fixed rule. Genuinely consider both "
    "possibilities before concluding:\n"
    "- INTENTIONAL: the page's content is non-substantive for a knowledge "
    "wiki — a running header/footer, a bare page break, a table-of-contents "
    "entry, whitespace, or content that purely restates something already "
    "filed under a different page's concept (check with search_wiki/grep_index "
    "before assuming this).\n"
    "- GENUINE_MISS: the page contains real spec content that should have "
    "its own or an updated concept entry but doesn't.\n\n"
    "If GENUINE_MISS: check_duplicates first, then actually file it with "
    "write_wiki_page (full OKF frontmatter per the schema below) rather than "
    "only reporting the gap — you have the same tools entity-ingest-agent "
    "has. If INTENTIONAL: do not create a page for it.\n\n"
    "Either way, call log_coverage_review exactly once per page with your "
    "real reasoning (not a template sentence) and what action you took.\n\n"
    + SCHEMA_TEXT
)

coverage_reviewer_subagent = {
    "name": "coverage-reviewer-agent",
    "description": (
        "Reviews a page the wiki appears to have skipped: given the page's "
        "raw source content, reasons about whether the omission was "
        "intentional (boilerplate, headers, pure restatement) or a genuine "
        "miss, self-heals genuine misses by filing the missing concept, and "
        "always logs its reasoning via log_coverage_review."
    ),
...
agent = create_deep_agent(
    ...
    backend=backend,
    subagents=[entity_ingest_subagent, coverage_reviewer_subagent],
)
...
def chunk_sections(sections: list[dict], max_chars: int | None = None) -> tuple[list[list[dict]], list[str]]:
    ...
    A single section's content is NEVER split across two chunks — each
    section dict is always appended whole. If one section alone is bigger
    than the whole budget, it's still placed alone in its own chunk (there's
    no way to process it otherwise without breaking it apart, which would be
    worse), but this is now surfaced explicitly rather than silently
    overflowing: returns (chunks, oversized_section_ids) so callers can flag
    it in logs/coverage reasoning instead of it passing unnoticed.
    """
    if max_chars is None:
        max_chars = available_context_budget() * 4  # tokens -> rough chars
    chunks, current, current_len = [], [], 0
    oversized_section_ids: list[str] = []
    ...
    for sec in sections:
        ...
        if size > max_chars:
            ...
            oversized_section_ids.append(sec_id)
            print(f"[chunk_sections] WARNING: section {sec_id} alone is {size} chars, "
                  f"over the {max_chars}-char budget — processing it alone rather than "
                  "silently merging/overflowing, but its results should get extra "
                  "scrutiny (see coverage review notes for this chunk)")
        current.append(sec)
        current_len += size
    if current:
        chunks.append(current)
    return chunks, oversized_section_ids
...
def ingest_large(source_path: Path) -> None:
    ...
    Coverage checking now happens PER CHUNK, right after that chunk's
    ingestion, instead of only as a post-hoc pass over the finished wiki:
    check_chunk_coverage() finds pages this chunk expected but that no wiki
    page claims yet, and review_chunk_coverage() delegates each gap to the
    coverage-reviewer-agent subagent for actual reasoning (intentional
    omission vs. genuine miss) and, where appropriate, self-healing —
    logged permanently via log_coverage_review, not just left in this
    turn's transcript.
    """
    sections = json.loads(source_path.read_text(encoding="utf-8"))
    manifest = build_manifest(sections)
    chunks, oversized_section_ids = chunk_sections(sections)
    if oversized_section_ids:
        print(f"[ingest-large] {len(oversized_section_ids)} section(s) exceeded the "
              f"chunk budget alone and were processed solo: {oversized_section_ids}")
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        # Mid-pipeline coverage check — right after this chunk, not at the end.
        gaps = check_chunk_coverage(chunk, oversized_section_ids)
        review_chunk_coverage(gaps, chunk_index=i)
...
def main():
    ...
    if cmd == "ingest-large":
        ...
        try:
            source_sections = json.loads(source_text)
            if isinstance(source_sections, list):
                gaps = check_chunk_coverage(source_sections, oversized_section_ids=[])
                review_chunk_coverage(gaps, chunk_index=1)
        except json.JSONDecodeError:
            pass  # plain-text sources (per the docstring) have no page-level structure to check
